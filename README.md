...
a functional classroom project with a local database and basic authentication. Through three enhancements, I have transformed 
it into something closer to a production application. In a demonstration of good software design and engineering, I have replaced 
the local credential storage with Firebase Authentication and implemented role-based access control (RBAC). In an application 
of the principles of algorithms and data structures, I have implemented search, filter, and sort features using memory- and
processor-efficient algorithms evaluated for time complexity as well as reliability, maintainability, and security. Finally,
I have moved the inventory database from local storage to the cloud, Firebase Firestore specifically, making the application
finally truly multi-user in nature.
